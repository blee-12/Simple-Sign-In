export function validateAndTrimString(
  ...
  if (minLength && str.length < minLength)
    throw new Error(
      `Argument '${label}' must be at least ${minLength} chars long`
    );
  if (maxLength && str.length > maxLength)
    throw new Error(
      `Argument '${label}' must be at most ${maxLength} chars long`
    );
  return str;
}
...
export function validateEmail(email: string) {
  ...
  const regex =
    /^(?:[A-z1-9]+[.-])*[A-z1-9]+@(?:[A-z1-9.-]+[.-])*[A-z1-9]+\.[A-z1-9]+$/;
...
export function validateStrAsObjectId(id: string, label = "ID") {
  ...
  if (!/^[0-9a-fA-F]{24}$/.test(id))
    throw new Error(`${label} does not represent a valid ObjectId string`);
  return id;
}
